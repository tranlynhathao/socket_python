...
def download_file(client_socket, filename, priority, progress, total_size, lock):
...
def print_progress(progress, total_size, lock):
...
def request_files(client_socket, files_list, last_requested_files, lock):
    try:
        ...
        if requested_files != last_requested_files:
            ...
            for requested_file in requested_files.split("\n"):
                if requested_file.strip():
                    ...
                    file_info = [file for file in files_list.split("\n") if file.startswith(filename)]
                    if file_info:
                        total_size[filename] = int(file_info[0].split()[1][:-2]) * 1024 * 1024
                        progress[filename] = 0
                        t = threading.Thread(target=download_file, args=(client_socket, filename, priority, progress, total_size, lock))
                        threads.append(t)
                        t.start()

            progress_thread = threading.Thread(target=print_progress, args=(progress, total_size, lock))
...
def main():
    ...
    try:
        ...
        lock = threading.Lock()
        while True:
            last_requested_files = request_files(client_socket, files_list, last_requested_files, lock)
            time.sleep(2)
